...
*/
bool connectFourBoard::isBoardEmpty() {
    for (int i = 0; i < row_size; i++) {
        for (int j = 0; j < col_size; j++) {
            if (board[i][j] != ' ')
                return false;
...
/* this function is used to
   ...
   choosing a column
*/
int connectFourBoard::makeFirstMove() {
    int random_col;
    srand(time(NULL));
    random_col = rand() % 7 + 1;
    if (random_col == 7)
...
*/
void connectFourBoard::checkPossibleMoves() {
    int c = 0;
...
*/
bool connectFourBoard::checkPlayerWon() {
    bool win = false;
    char c1, c2, c3, c4;
...
*/
void connectFourBoard::operator=(connectFourBoard rhs) {
    for (int i = 0; i < row_size; i++) {
        for (int j = 0; j < col_size; j++) {
...
/* this function is used for displaying
   board contents
*/
void connectFourBoard::display_board(connectFourBoard* x) {
    cout << endl;
    cout << "C0 "
         << " C1 "
         << " C2 "
